...
function getStoredDocuments(): UploadedDocument[] {
  ...
  const stored = localStorage.getItem('veyra_documents');
  return stored ? JSON.parse(stored) : [];
}

function saveDocuments(docs: UploadedDocument[]): void {
  localStorage.setItem('veyra_documents', JSON.stringify(docs));
  localStorage.setItem('veyra_docs_uploaded', docs.length > 0 ? 'true' : 'false');
}
...
function getSessionId(): string {
  ...
  let sessionId = localStorage.getItem('veyra_session_id');
  if (!sessionId) {
    sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    localStorage.setItem('veyra_session_id', sessionId);
  }
  return sessionId;
}
...
export default function VeyraApp() {
  ...
  const handleClearDocuments = async () => {
    if (confirm('Are you sure you want to clear all uploaded documents? This will reset your session.')) {
      ...
      localStorage.removeItem('veyra_documents');
      localStorage.removeItem('veyra_docs_uploaded');
      localStorage.removeItem('veyra_session_id');
      setDocuments([]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col">
        ...
        <header className="bg-white border-b border-gray-200 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Veyra</h1>
        ...
        <footer className="bg-white border-t border-gray-200 py-4">
          <div className="max-w-7xl mx-auto px-4 text-center text-sm text-gray-500">
            Veyra Backend v0.0.0 • Powered by FastAPI + Graph-RAG
          </div>
        </footer>
      </div>
    </>
